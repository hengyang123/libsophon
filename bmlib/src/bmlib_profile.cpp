...
static const char* get_dir(bool is_src_local, bool is_dst_local){
    const char* dirs[] = {"S2S", "S2L", "L2S", "L2L"};
    return dirs[(is_src_local<<1) | is_dst_local];
}
...
void show_bm1684_gdma_item(const BM1684_GDMA_PROFILE_FORMAT* data, int len, float freq_MHz, float time_offset=0){
  ...
  for(int i=0; i<real_print; i++){
    ...
    const char* dir = get_dir(p.gif_ar_bytes>0, p.gif_wr_bytes>0);
...
static bm_status_t __bm1688_set_engine_profile_param(bm_handle_t handle, int core, int engine_type, unsigned long long addr, unsigned long long size){
  ...
  param.size = size;
#endif
  bm_status_t status = bm_send_api_to_core(handle, BM_API_ID_SET_ENGINE_PROFILE_PARAM, (u8*)&param, sizeof(param), core);
  if (BM_SUCCESS != status) {
...
static bm_status_t __bm1688_set_profile_enable(bm_handle_t handle, unsigned enable_bits, int core_id){
  ...
  bm_status_t status = bm_send_api_to_core(handle, BM_API_ID_SET_PROFILE_ENABLE, (const u8*)&profile_enable, api_buffer_size, core_id);
  if (BM_SUCCESS != status) {
...
void show_bm1688_gdma_item(const BM1688_GDMA_PROFILE_FORMAT* data, int len, float freq_MHz, int core_id, float time_offset=0){
  ...
  for(int i=0; i<real_print; i++){
    ...
    auto speed = total_time>0?total_bytes/total_time: -1000.0;
    const char* dir = get_dir(p.gif_fmem_ar_bytes>0, p.gif_fmem_wr_bytes>0);
...
#pragma pack()
  ...
  bm_status_t status = bm_send_api_to_core(handle, BM_API_ID_GET_PROFILE_DATA,
                                           (const u8*)&api_data, sizeof(api_data), core_id);
  if (BM_SUCCESS != status) {
    PROFILE_ERROR("%s tpu kernel launch failed, status:%d\n", __func__, status);
  }
  return status;
...
void bm_profile_load_module(bm_handle_t handle, tpu_kernel_module_t p_module, int core_id) {
    ...
    if(!profile->cores[core_id].current_module){
        ...
        profile->cores[core_id].enable_profile_func_id = tpu_kernel_get_function(handle, p_module, "sg_api_set_profile");
        profile->cores[core_id].get_profile_func_id = tpu_kernel_get_function(handle, p_module, "sg_api_get_profile_data");
        ASSERT(handle->profile->cores[core_id].enable_profile_func_id);
        ASSERT(handle->profile->cores[core_id].get_profile_func_id);
        PROFILE_INFO("Profile load module: %s\n", p_module->lib_name);
...
void bm_profile_unload_module(bm_handle_t handle, tpu_kernel_module_t p_module, int core_id) {
    ...
    PROFILE_INFO("Profile unload module: %s\n", p_module->lib_name);
    bm_profile_dump_core(handle, core_id);
